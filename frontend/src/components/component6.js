...
const Component6 = (propss) => {
    const ImageBox = (props) => <> <div style={{ left: '50%', transform: 'translate(-50%)', margin: '0 auto', width: '65%', textAlign: 'center', position: 'absolute', height: '50px', display: 'inline-block', lineHeight: '53px', fontWeight: '700', borderRadius: '100px', fontSize: '25px', backgroundColor: '#008000', color: '#ffffff', zIndex: '100' }} >{props.data.Keyword1}</div>
                    ...
                    <ul style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', columnGap: '50px', listStyle: 'none' }} >
                        <li style={{ display: 'list-item', position: 'relative' }}>
                            ...
                            <div style={{ marginTop: '30px', display: 'block' }} >
                                ...
                                }} >{propss.data.data.Title4Words}</h3>
                                <p style={{
                                ...
                                }}>{propss.data.data.SummaryShort.map((content) => <>{content}<br /></>)}</p>
                            </div>
                        </li>
                        <li style={{ display: 'list-item', position: 'relative' }}>
                            <ImageBox imgheader={propss.data.data.Keyword2} imgurl={propss.data.imgurl2} />
                            <div style={{ marginTop: '30px', display: 'block' }} >
                                <h3 style={{
                                    fontSize: '30px', fontWeight: '600', lineHeight: '1.41', overflow: 'hidden', textOverflow: 'clip',
                                    display: '-webkit-box', webkitBoxOrient: 'vertical', padding: '0px', margin: '0px'
                                }} >{propss.data.data.Title3Words}</h3>
                                <p style={{
                                    fontSize: '20px', paddingBottom: '1px', fontWeight: '400', lineHeight: '1.87',
                                    display: '-webkit-box', webkitBoxOrient: 'vertical', color: '#666', textOverflow: 'clip'
                                }}>{propss.data.data.SummaryShort2.map((content) => <>{content}<br /></>)}</p>
                                <div style={{ marginTop: '30px', marginBottom: '-20px' }} >
                                    ...
                                    }} href={propss.data.data.WebpageURL} target="_blank" rel="noopener">
                                        <span>자세히 보기</span>
