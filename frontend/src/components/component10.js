...
const Component10 = (props) => {
    ...
    const Img = () => <img style={{ width: '550px', aspectRatio: ' 476 / 230', objectFit: 'cover', overflow: 'hidden' }} src={props.data.imgurl[0]}></img>
    const Content = () => <div style={{
        ...
        <p style={{ fontWeight: 'bold', fontSize: '37px', margin: '0px 0px 10px 0px' }}>{props.data.Keyword1}</p>
        <p style={{ fontWeight: 'lighter', lineHeight: '30px', fontSize: '19px' }}>{props.data.ExplainIn2sentence.map((content) => <>{content}<br /></>)}</p>
    </div>
    return (
